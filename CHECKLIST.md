...
- [ ] **Bot IA**: ✅ funcionando en producción (gpt-oss-120b) · Etapa C COMPLETA (bandeja web) · **Etapa E COMPLETA 09-sep**: 📸 fotos como evidencia (con texto = borrador nuevo; reply al resumen = adjuntar a ESE; suelta = último pendiente) directo a Supabase Storage, botones inline [✅/❌/🏢 Cambiar empresa] con idempotencia, `/empresa` determinista (fuzzy contra entidades + ensure-portfolio), voz nueva también al bucket, y rechazo de grupos. E2E a nivel driver verificado (draft 48). Quedan B.5 (RAG) y D/F
